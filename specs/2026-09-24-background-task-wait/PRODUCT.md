...
Raising the wait at settlement also strands work. Measured live in a clean session: a goal started a 120 s background task, wrote file step 1, ended its turn, and the record read `phase: "waiting"` with that work still pending; it had not run 3+ minutes later. Independent work the run left undone sat behind a task it did not depend on.

## Behaviour

A tool result that leaves work running records a scheduler wait in place of the next automatic run, once the run has no more work that does not depend on the task. The user sees the widget report `Waiting` with the tracked task and the next check, and the goal stays quiet until the task reports.
...
2. **Wait, once the run stops making progress.** At settlement the goal records a wait on the task and stops scheduling runs. A run that did work after the detection does not sleep on the task: it dispatches one more run, and this repeats while each run keeps progressing. The wait is raised on the first settle that adds nothing new, which is what bounds it. Progress is mechanical, not a model judgement: any tool result that arrives after the task was recorded, excluding goal bookkeeping (`create_goal`, `get_goal`, `update_goal`) and excluding a refused poll of the tracked task. The result that detected the task never counts, so starting a task and stopping still waits. The wait is system-managed: it needs no setting and does not put the goal under the explicit execution contract.
...
5. **Steer.** The producer result gains a `[PI GOAL WAITING ON BACKGROUND TASK]` block, and the injected state carries a standing wait block, so the model stops instead of polling. Both now also tell the model to finish work that does not depend on the result, and to declare `update_goal({ continuation: { kind: "ready", next_action: "<what remains>" } })` when work remains for a later turn. Previously the notice only said to end the turn, which is what stranded independent work.
...
- **A wait is deferred while a run keeps working.** Raising the wait at settlement stranded independent work the run left undone behind a task it did not depend on, so the wait is raised only once a run settles without new work. Progress is measured mechanically from tool results, never from the model's own account of its work, and the detecting result itself never counts.
