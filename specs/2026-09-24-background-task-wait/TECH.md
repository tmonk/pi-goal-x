...
- `noteToolResult(event)` is memory-only. It records the pending task, clears it when a result reports the same task settled, and stays inert unless `canWaitForTask()` (focused goal `status === "active"` and `autoContinue`) holds.
- `settled()` calls `deferForTask()` in place of `implicitReady()` when a task is pending. The agent's own declaration still wins, and the availability check and repair path are untouched.
...
- `begin()`, `takeover()`, `restore()`, and `shutdown()` clear the per-run pending task.
- `blocksTaskPoll(toolName, input)` refuses a second wait on the tracked task while the goal can wait.

`extensions/goal-scheduler-state.ts`: `GoalWait.taskId?` with normalization, and the scheduler summary. A task wait is folded into the runs line (`Autonomous runs: N/unlimited. Waiting for pwsh task ps_… to finish. Next check <iso>.`) so the widget gains no rows. Declared waits keep their existing two-row form.

## Events and prompts

`extensions/goal-events.ts`:

- `tool_result` calls `noteToolResult` and appends `backgroundTaskNotice(task)` to the producer result when a task is tracked.
...
- Waiting consumes no autonomous run until the re-check or the wake actually dispatches.
...
`tests/goal-background-task-pause.test.ts`:
...
- `npm run test:all`: 1096 tests, 1077 pass, 17 fail. The upstream tree with the same runner reports 1086 / 1067 / the identical 17 names, so the delta is the 11 new tests and there is no regression. Those 17 are Windows environment failures (symlinks, POSIX path resolution, CRLF goldens, and the recover script).
